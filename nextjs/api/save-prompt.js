// API endpoint for saving user prompts
export default async function handler(req, res) {
  // Set CORS headers
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  
  // Handle preflight requests
  if (req.method === 'OPTIONS') {
    res.status(200).end();
    return;
  }

  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    const { username, promptName, prompt } = req.body;

    // Validate required fields
    if (!username || !promptName || !prompt) {
      return res.status(400).json({ 
        error: 'Missing required fields: username, promptName, and prompt are required' 
      });
    }

    // Validate input lengths
    if (username.length > 100) {
      return res.status(400).json({ error: 'Username too long (max 100 characters)' });
    }
    
    if (promptName.length > 200) {
      return res.status(400).json({ error: 'Prompt name too long (max 200 characters)' });
    }
    
    if (prompt.length > 5000) {
      return res.status(400).json({ error: 'Prompt too long (max 5000 characters)' });
    }

    // Sanitize inputs
    const sanitizedUsername = username.trim().toLowerCase();
    const sanitizedPromptName = promptName.trim();
    const sanitizedPrompt = prompt.trim();

    // In a real app, you would save to a database here
    // For now, we'll simulate saving to a simple in-memory store
    // In production, use MongoDB, PostgreSQL, or other database
    
    console.log('Saving prompt:', {
      username: sanitizedUsername,
      promptName: sanitizedPromptName,
      prompt: sanitizedPrompt,
      timestamp: new Date().toISOString()
    });

    // Simulate successful save
    res.status(200).json({ 
      success: true, 
      message: 'Prompt saved successfully',
      data: {
        username: sanitizedUsername,
        promptName: sanitizedPromptName,
        savedAt: new Date().toISOString()
      }
    });

  } catch (error) {
    console.error('Error saving prompt:', error);
    res.status(500).json({ 
      error: 'Internal server error',
      message: 'Failed to save prompt'
    });
  }
}